package by.it.medved.util;

public final class FieldsEntities {

    public static final String USER = "user";
    public static final String SELECTED_USER = "selectedUser";
    public static final String VERIFIED_USER = "verifiedUser";
    public static final String SELECTED_MOVIE = "selectedMovie";
    public static final String SHOW_USERS = "showUsers";
    public static final String EDIT_PROFILE = "editProfile";
    public static final String SHOW_TICKETS = "showTickets";
    public static final String RETURN_USER_TICKET = "returnUserTicket";
    public static final String EDIT_ROLE = "editRole";
    public static final String EDIT_MOVIE = "editMovie";
    public static final String DELETE_MOVIE = "deleteMovie";
    public static final String SHOW_MOVIES = "showMovies";
    public static final String ADD_MOVIE = "addMovie";
    public static final String BUY_TICKET = "buyTicket";
    public static final String NO_TICKETS = "noTickets";
    public static final String ACTION = "action";
    public static final String MESSAGE = "message";
    public static final String ERROR_MESSAGE = "errorMessage";
    public static final String UPDATE_PROFILE_SUCCESSFUL = "updateProfileSuccessful";
    public static final String UPDATE_MOVIE_SUCCESSFUL = "updateMovieSuccessful";
    public static final String SING_IN = "Sing In";
    public static final String REGISTER = "Register";
    public static final String EDIT_USER = "editUser";
    public static final String USERS = "users";
    public static final String MOVIES = "movies";
    public static final String TICKETS = "tickets";
    public static final String USER_TICKETS = "userTickets";
    public static final String USER_ID = "userId";
    public static final String LOGIN = "login";
    public static final String ENTERED_LOGIN = "enteredLogin";
    public static final String PASSWORD = "password";
    public static final String ROLE = "role";
    public static final String FIRST_NAME = "firstName";
    public static final String EMAIL = "email";
    public static final String DATE_BIRTHDAY = "dateBirthday";
    public static final String MOVIE_ID = "movieId";
    public static final String TICKET_ID = "ticketId";
    public static final String MOVIE_TITLE = "movieTitle";
    public static final String SHOW_DATE_TIME = "showDateTime";
    public static final String SHOW_DATE = "showDate";
    public static final String SHOW_TIME = "showTime";
    public static final String PRICE = "price";
    public static final String AGE_LIMIT = "ageLimit";
    public static final String DELETE = "delete";
    public static final String BACK = "back";
    public static final String EXIT = "exit";

    private FieldsEntities() {
    }
}